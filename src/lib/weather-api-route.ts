import { fetchKmaWeather } from "./kma-short-term-forecast";
...
export async function handleWeatherApiRequest(
  request: Request,
  env: unknown,
  ...
  try {
    const weather = await fetchKmaWeather({ lat, lng, serviceKey, fetchImpl });
    return jsonResponse<WeatherData>({
      success: true,
      data: weather,
      cached: false,
    ...
    return jsonResponse<WeatherData>(
      {
        success: false,
        data: null,
        error: error instanceof Error ? error.message : "기상청 단기예보 조회에 실패했습니다.",
      },
      502,
    );
  }
